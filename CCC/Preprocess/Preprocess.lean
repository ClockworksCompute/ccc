...
private def stripWS (s : String) : String :=
  String.ofList (s.toList.dropWhile (fun (c : Char) => c == ' ' || c == '\t'))
...
where
  processDirective (st : PPState) (directive : String) : IO PPState := do
    if directive.startsWith "ifdef " then
      ...
      pure { st with ifStack := (st.isActive && defined) :: st.ifStack }
      ...
      -- Simplified: check "defined(X)" or "0"/"1"
      let expr := stripWS (dropN directive 3)
      let result := if st.isActive then evalSimple st expr else false
      pure { st with ifStack := result :: st.ifStack }
    else if directive.startsWith "elif " then
      match st.ifStack with
      | prev :: outer =>
          let expr := stripWS (dropN directive 5)
          let outerActive := outer.all (fun (b : Bool) => b)
          let result := outerActive && !prev && evalSimple st expr
          pure { st with ifStack := result :: outer }
      | [] => pure st
    else if directive.startsWith "else" then
      match st.ifStack with
      | prev :: outer =>
          let outerActive := outer.all (fun (b : Bool) => b)
          pure { st with ifStack := (outerActive && !prev) :: outer }
      | [] => pure st
  ...
  evalSimple (st : PPState) (expr : String) : Bool :=
    let expr := stripWS expr
    -- Handle && (split on first occurrence outside parens)
    if splitOnOp expr "&&" |>.isSome then
      match splitOnOp expr "&&" with
      | some (l, r) => evalSimple st l && evalSimple st r
      | none => false
    else if splitOnOp expr "||" |>.isSome then
      match splitOnOp expr "||" with
      | some (l, r) => evalSimple st l || evalSimple st r
      | none => false
    else if expr.startsWith "!defined" then
      let rest := stripWS (dropN expr 8)
      let name := if rest.startsWith "(" then
          firstWord (stripWS (dropN rest 1))
        else firstWord rest
      !(st.isDefined name)
    else if expr.startsWith "defined" then
      ...
      !(evalSimple st (stripWS (dropN expr 1)))
    else if expr == "0" then false
    else if expr == "1" then true
    else st.isDefined (firstWord expr)
  splitOnOp (s : String) (op : String) : Option (String × String) :=
    let chars := s.toList
    let opLen := op.length
    findOp chars op.toList opLen [] 0
  findOp (chars : List Char) (opChars : List Char) (opLen : Nat)
         (acc : List Char) (depth : Nat) : Option (String × String) :=
    match chars with
    | [] => none
    | '(' :: rest => findOp rest opChars opLen ('(' :: acc) (depth + 1)
    | ')' :: rest => findOp rest opChars opLen (')' :: acc) (if depth > 0 then depth - 1 else 0)
    | c :: rest =>
        if depth == 0 && prefixMatch (c :: rest) opChars then
          some (String.ofList acc.reverse, String.ofList ((c :: rest).drop opLen))
        else
          findOp rest opChars opLen (c :: acc) depth
  prefixMatch (chars : List Char) (pref : List Char) : Bool :=
    match pref, chars with
    | [], _ => true
    | _, [] => false
    | p :: ps, c :: cs => p == c && prefixMatch cs ps
