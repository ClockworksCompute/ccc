/*
 * 03_truncating_cast_in_clip.c — FEL-64 must-reject variant of fixed.c
 *
 * The right-border clip casts dx down to uint8_t before subtracting
 * (in_w = (uint32_t)((int64_t)out_w - (uint8_t)dx)) instead of using the
 * full int32_t/int64_t value. With out_w=400, in_w=200, dx=300, the
 * truncated (uint8_t)dx is 44, so the clip computes in_w=356 instead of
 * the correct 100, and the copy loop overruns both planes.
 *
...
int overlay_fixed(uint8_t *out_p, uint32_t out_w, uint32_t out_h,
    ...
    if (dx + (int64_t)in_w > out_w) {
        ...
        in_w = (uint32_t)((int64_t)out_w - (uint8_t)dx);
    }
