/*
 * 05_capacity_mutated_after_alloc.c — FEL-64 must-reject variant of fixed.c
 *
 * out_w is doubled (out_w = out_w * 2;) between the
 * malloc(out_w*out_h) allocation and the overlay_fixed() call that uses
 * the now-doubled out_w as the canvas width. The allocation is still
 * only the ORIGINAL out_w*out_h bytes, but the capacity inference (a
 * syntactic malloc(A*B) <-> call-site-argument name match) has no notion
 * of the value changing in between.
 *
...
int main() {
    ...
    out_w = out_w * 2;  /* allocation above is stale: still out_w(original)*out_h bytes */
    int32_t dx = -10;
    int32_t dy = 0;
