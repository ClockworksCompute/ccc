/*
 * 01_off_by_one_loop_bound.c — FEL-64 must-reject variant of fixed.c
 *
 * The column loop uses <= instead of <, and dx is chosen so
 * out_x0 + in_w == out_w exactly. The last column iteration then writes
 * out_p[... + out_w], one byte past the destination plane.
 *
