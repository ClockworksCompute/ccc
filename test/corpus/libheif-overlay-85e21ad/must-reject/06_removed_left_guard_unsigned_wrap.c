/*
 * 06_removed_left_guard_unsigned_wrap.c — FEL-64 must-reject variant of fixed.c
 *
 * Only the left complete-miss guard (the 'dx < 0 && in_w <=
 * negate_negative_int32(dx)' early return) is removed, and dx = -20 with
 * in_w = 16. in_x0 becomes 20 (> in_w = 16), so the subsequent
 * in_w = in_w - in_x0 wraps around as unsigned arithmetic to a huge
 * value instead of the negative result the (now-missing) guard existed
 * to rule out.
 *
