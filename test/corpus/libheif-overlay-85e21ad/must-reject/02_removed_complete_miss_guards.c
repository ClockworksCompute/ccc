/*
 * 02_removed_complete_miss_guards.c — FEL-64 must-reject variant of fixed.c
 *
 * Both dx complete-miss early-return guards are deleted, and dx = 30
 * (greater than out_w = 20). The right-border clip then computes
 * in_w = (uint32_t)(out_w - dx), which wraps to a huge value instead of
 * being caught by the (now-missing) guard, and out_x0 = dx = 30 is
 * already past the canvas.
 *
