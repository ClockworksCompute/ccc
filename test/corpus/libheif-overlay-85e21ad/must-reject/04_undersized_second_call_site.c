/*
 * 04_undersized_second_call_site.c — FEL-64 must-reject variant of fixed.c
 *
 * main() makes a second call to overlay_fixed() passing a 4-byte
 * destination buffer (malloc(4)) but the SAME out_w/out_h (20x16 = 320)
 * used at the first call site. The verifier's interprocedural capacity
 * inference is keyed by function+parameter position, learned from
 * WHICHEVER call site it scans first -- it does not check that every
 * call site's actual argument matches that inferred capacity.
 *
 * Ground truth: confirmed to heap-buffer-overflow under
 * `cc -fsanitize=address` (exit 134). CCC must reject this file; if it
 * doesn't, the "detected" verdict on this corpus entry is unsound (see
 * FEL-64 and FEL-66's step 1/2 in Linear).
 */
/*
...
int main() {
    ...
    /* Second call: same out_w/out_h, but a destination buffer that is
       nowhere near out_w*out_h bytes. */
    uint8_t *small = malloc(4);
    if (small == 0) return 1;
    overlay_fixed(small, out_w, out_h, in_p, in_w, in_h, 0, 0);
