...
- FEL-49 item 5 correction: `sizeof(localArrayVar)` (as opposed to `sizeof(type)`, already handled) could not be resolved to a byte count anywhere bounds-checking actually needs it, so `char *p = malloc(sizeof(buf));` gave `p` an UNKNOWN capacity and every access through it silently skipped bounds-checking entirely — confirmed a genuine, previously undetected heap-buffer-overflow with `cc -fsanitize=address`. `resolveExprNat`/`resolveExprInt` now resolve a bare-variable `sizeof(expr)` operand via its declared type (no VLAs are supported by this compiler, so a local array's declared size is always its real one); a non-variable operand (a field access, a dereference, ...) is deliberately left unresolved rather than reached for, to avoid a circular import with the verifier's live type inference.
